                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('tenants.store') }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <x-text-input id="name" class="block mt-1 w-full bg-gray-100" type="text" name="name" :value="old('name', auth()->user()->name)" readonly />
                            <p class="mt-1 text-sm text-gray-500">Se usa automáticamente tu usuario master autenticado.</p>
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="email" :value="__('Email administrador')" />
                            <x-text-input id="email" class="block mt-1 w-full bg-gray-100" type="email" name="email" :value="old('email', auth()->user()->email)" readonly />
                            <p class="mt-1 text-sm text-gray-500">Este correo queda vinculado al tenant creado.</p>
